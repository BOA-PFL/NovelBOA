...
for s = 1:NumbFiles
   ...
   %make vectors of left or right and configuration name to add into file
    ...
    forceDat.(configName).LTot = LTot; 
    forceDat.(configName).LHeel = LHeel; 
    forceDat.(configName).LLat = LLat; 
    forceDat.(configName).LMed = LMed;
    forceDat.(configName).RTot = RTot; 
    forceDat.(configName).RHeel = RHeel; 
    forceDat.(configName).RLat = RLat; 
    forceDat.(configName).RMed = RMed;
...
if writeData == 1
    ...
    writetable(T,'SHlace.csv')
end
% 
%% make plots 
% 
subplot(2,2,1)
shadedErrorBar(0:desiredStepLength, forceDat.DD.RTot(1:end-1,:), {@mean, @std}, 'lineprops','blue');
hold on
shadedErrorBar(0:desiredStepLength, forceDat.Lace.RTot(1:end-1,:), {@mean, @std}, 'lineprops','red');
title('Total Force: DD (blue) vs. Lace (red)')
ylabel('Force (N)')
ylim([0 2000])

subplot(2,2,2)
shadedErrorBar(0:desiredStepLength, forceDat.DD.RHeel(1:end-1,:), {@mean, @std},'lineprops', 'blue')
hold on
shadedErrorBar(0:desiredStepLength, forceDat.Lace.RHeel(1:end-1,:), {@mean, @std},'lineprops', 'red')
title('DD (blue) vs. Lace (red) heel')
ylim([0 1000])

subplot(2,2,3)
shadedErrorBar(0:desiredStepLength, forceDat.DD.RMed(1:end-1,:), {@mean, @std},'lineprops', 'blue')
hold on
shadedErrorBar(0:desiredStepLength, forceDat.Lace.RMed(1:end-1,:), {@mean, @std},'lineprops', 'red')
title('DD (blue) vs. Lace (red) medial forefoot')
ylabel('Force (N)')
xlabel('Time (cs)')
ylim([0 1000])

subplot(2,2,4)
shadedErrorBar(0:desiredStepLength, forceDat.DD.RLat(1:end-1,:), {@mean, @std},'lineprops', 'blue')
hold on
shadedErrorBar(0:desiredStepLength, forceDat.Lace.RLat(1:end-1,:), {@mean, @std},'lineprops', 'red')
title('DD (blue) vs. Lace (red) lateral forefoot')
xlabel('Time (cs)')
ylim([0 1000])